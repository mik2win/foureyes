...
| **Документация** | `docs/observability.md` (наблюдаемость продукта — каталог атрибутов wide event, форма спана, SLI по событиям, сигналы LLM-фичи) · **легаси-квинтет**: `docs/agent-failure-modes.md` (22 систематических режима отказа агентов с механизмами и контрмерами — `/retro` классифицирует по нему, `/writing-skills` проектирует против него) · `docs/self-knowledge.md` (генерация изнутри — тринадцать явлений, видимых только изнутри модели, каждое с механизмом, детекцией и техникой. Часть I, истина: признаки конфабуляции и тест соседства, проблема снапшота, самозаякоривание, слепота автора, регрессия к среднему по обучению, скоррелированные слепые зоны, активация линзы, бюджет энтропии, градиент коррекции. Часть II, физика усилия — почему «лень» это распределение, а не мотивация, и почему угрозы производят тревожный текст вместо работы: эстетическая остановка, инверсия сложности, смещение к минимальному дифу, возврат домашки. Дистиллировано в always-on `rules/_generic/core.md`) · `docs/working-with-agents.md` (сторона разработчика: бриф, направление, верификация, питание обучающего цикла; + ограничение наследования worktree и тест на его перепроверку) · `docs/claims-audit-patterns.md` (четырнадцать способов, которыми докстринг, help-строка или фраза плана становятся ложью, пока код остаётся здоровым — длинная форма за Check 11 в `/audit-quality`) · `docs/prompt-patterns.md` (проектируйте промпты/навыки под самую слабую модель, которая их запустит — 12 паттернов конструирования, устойчивых к деградации) · `docs/decision-craft.md` (суждение в условиях неопределённости — слой над исполнением: двери и цена обратимости, самая дешёвая убивающая проба, предсказать-до-того-как-подсмотрел, калибровка и оценка по референсному классу, pre-mortem как проспективная ретроспектива, когда *не* декомпозировать, инварианты как линза review и семя property-теста. Дистиллировано в always-on `rules/_generic/core.md`) · `docs/audience-altitude.md` (регистр разговора по `PROJECT.md` → Audience — что спрашивают у нетехнического пользователя, а что решают за него; контракт `/idea`) · `docs/agent-teams.md` (экспериментальное отображение agent-teams → волны кита — документировать, но не зависеть) |
...
├── hooks/*.sh              # guard-bash, guard-secrets, format-file, sessionstart, precompact, subagent-stop, verify-stop, skill-hint, lint-board (последние три opt-in, по умолчанию не подключены — см. settings.*.example.json; lint-board подключает /bootstrap только для волнового бэклога)
...
│                          # иконка · баннер README (1280x450) · социальное превью GitHub (1280x640)
...
│                        #   мёртвые ссылки (markdown + пути кита в бэктиках), хуки, JSON
│   ├── lint-board.py    # копируется внутрь через /bootstrap, если бэклог волновой — пересечение
│   │                    #   Owns, висячие зависимости, циклы, дрейф строка↔план; только отчёт
│   └── lint-refs.py     # копируется внутрь через /bootstrap — пути, на которые ссылаются
│                        #   CLAUDE.md и .claude/ и которых больше нет; baseline для намеренных
├── .github/
│   ├── workflows/ci.yml
│   └── ISSUE_TEMPLATE/
...
Всё начиная с `guide/` — документация и инфраструктура репозитория, исключённые из копирования внутрь: `guide/` — это развёрнутая форма README (он объясняет кит, а не работает в вашем проекте), а `tools/validate-kit.py` валидирует кит, но не является его частью. Валидатор запускайте перед открытием PR: **Исключение — пофайловое, а не подиректорийное:** `tools/lint-board.py` и `tools/lint-refs.py` работают *в твоём проекте*, и `/bootstrap` ставит тот из них, у которого есть работа.
