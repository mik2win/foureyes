...
├── hooks/*.sh              # guard-bash, guard-secrets, format-file, sessionstart, precompact, subagent-stop, verify-stop, skill-hint, lint-board (last three opt-in, unwired by default — see settings.*.example.json; lint-board is wired by /bootstrap only for a wave-structured backlog)
├── output-styles/review.md
├── schemas/finding.schema.json   # the Finding Contract as JSON Schema (structured fan-out output)
├── docs/{observability,agent-failure-modes,self-knowledge,decision-craft,working-with-agents,prompt-patterns,audience-altitude,agent-teams,claims-audit-patterns}.md
...
│                        #   dead links (markdown + backticked kit paths), hooks, JSON
│   ├── lint-board.py    # copied in by /bootstrap when the backlog is wave-structured — Owns
│   │                    #   overlap, dangling deps, cycles, row↔plan status drift; report-only
│   └── lint-refs.py     # copied in by /bootstrap — repo paths cited by CLAUDE.md and .claude/
│                        #   that no longer exist, with a baseline for the deliberate ones
├── .github/
│   ├── workflows/ci.yml
│   └── ISSUE_TEMPLATE/
...
Everything from `guide/` down is documentation and repo infrastructure, excluded from the copy-in: `guide/` is the README's long form (it explains the kit, it doesn't run in your project), and `tools/validate-kit.py` validates the kit rather than being part of it. **The exclusion is per file, not per directory** — `tools/lint-board.py` and `tools/lint-refs.py` run *in your project*, and `/bootstrap` installs whichever of them has work to do. Run the validator before opening a PR:
