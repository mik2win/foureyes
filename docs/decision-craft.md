...
- **Rate is part of reversibility.** For automation that destroys — deletes rows, stops instances, revokes access, rewrites history — the door is not the code but how much of it runs before a person can look: cap the rate and the fraction per interval on the destroying direction while leaving the creating direction fast, and name *both* bounds, because a governor too slow to react is its own failure. Unbounded automatic destruction is a one-way door nobody priced as one.
