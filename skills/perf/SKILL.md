...
- **One metric, named precisely**: p95 latency of endpoint X / wall-time of job Y / RSS
  after N ops / cold-start / throughput — not synonyms. Throughput trades against response
  time, and responsiveness (how fast the request is acknowledged) is a third axis, answered
  by a job id and progress, not by shorter work. "Faster" is not a metric; ask which is meant.
...
- **Floor before the profile**: serial waits on the path add up; parallel ones cost the
  slowest alone. Floor already over budget → escalate per Phase 5, don't profile.
...
Build it at production's construct-to-call ratio — the wrong ratio validates the opposite fix.
...
If the work is a graph rather than a line (pipeline, fan-out, parallel jobs), the profile is
the critical path through it: time cut off that path changes the total by nothing.
...
**Put the compromise in the effect layer.** Batching, paging and caching belong in the code
that already does I/O — keep domain functions phrased over "zero or more X". Purity is not
the test: one that grew a `page_size` or a cursor is still pure and now knows the storage.
...
session, and a key missing anything the answer depends on (including what the handler mixes
in while building it) serves one user's data to the next. Name the reads that must never be
cached — user-scoped, time-sensitive. The same question is owed by everything else a change
accumulates — rows, log files, files on disk, an in-memory seen-set: name what reclaims it,
in this change. An unbounded key space is not yet a cache.
**Precomputed derived data has two paths.** Precompute for the rows read often enough to pay
for the write, compute on read for the tail — then say which rows fall on which side, and what
a read costs when the hot ones turn out to be in the tail. That case decides the split.
...
Changes kept: <one line each — lever, expected vs actual effect, domain signature it changed>
